...
function applyVisiblePricing(product: any) { return normalizeVisibleCjProduct(product); }
...
export async function createOrder(req: AuthedRequest, res: Response, next: NextFunction) {
  try {
    ...
    const order = await prisma.order.create({ data: { orderNumber, userId, addressId, subtotal, discountTotal, shippingTotal, grandTotal, couponId: coupon?.id, items: { create: visibleItems.map((item) => ({ productId: item.productId, variantId: item.variantId ?? undefined, titleSnapshot: item.product.title, unitPrice: Number(item.product.sellingPrice) + Number(item.variant?.priceDelta ?? 0), quantity: item.quantity })) } }, include: { items: true } });
    const paymentSession = await createPaymentIntent(paymentProvider, grandTotal, order.orderNumber, { customerEmail: user.email, metadata: { orderId: order.id } });
    const payment = await prisma.payment.create({ data: { orderId: order.id, provider: paymentProvider, amount: grandTotal, currency: paymentProvider === 'PAYSTACK' ? 'NGN' : 'USD', providerRef: paymentSession.reference } });
...
export async function listMyOrders(req: AuthedRequest, res: Response, next: NextFunction) {
  try { const orders = await prisma.order.findMany({ where: { userId: req.user!.sub }, orderBy: { createdAt: 'desc' }, include: { items: true, payments: true } }); res.json({ orders }); } catch (err) { next(err); }
}

export async function getOrderById(req: AuthedRequest, res: Response, next: NextFunction) {
  try { const order = await prisma.order.findFirst({ where: { id: req.params.id, userId: req.user!.sub }, include: { items: true, payments: true, address: true } }); if (!order) throw new AppError('Order not found.', 404); res.json({ order }); } catch (err) { next(err); }
}

export async function updateOrderStatus(req: AuthedRequest, res: Response, next: NextFunction) {
  try { const { status, trackingNumber, trackingCarrier } = req.body; const order = await prisma.order.update({ where: { id: req.params.id }, data: { status, trackingNumber, trackingCarrier }, include: { user: true } }); await sendShippingUpdateEmail(order.user.email, order.user.firstName, order.orderNumber, status, trackingNumber); res.json({ order }); } catch (err) { next(err); }
}